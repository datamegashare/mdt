...
//  v3 - Fix Google Doc: recibe textoPlano, sin HTML crudo.
//       Problema residual: todo en una línea (quill.getText()
//       no preserva estructura de listas), Calendar sin bullets.
//  v4 - Fix definitivo Doc y Calendar:
//       · El servidor parsea el HTML de Quill con parseHtmlATexto()
//       · Convierte <ol>/<ul>/<li> a texto con numeración o bullets
//       · Convierte <strong>/<em>/<u> a marcadores visuales
//       · El Doc recibe líneas bien formateadas, una por párrafo
//       · Calendar recibe el mismo texto limpio estructurado
//       · El HTML original sigue guardándose en el Sheet (para la UI)
//
...
//    Notas       → Fecha | FechaHora | Etiqueta | Contenido
...
// ─────────────────────────────────────────────────────────────
//  guardarNota — persiste en Sheet + Doc + Calendar  (v4)
//
...
function guardarNota({ email, etiqueta, contenido, textoPlano, fechaHora }) {
  ...
  const tituloEvento = etiqueta + ' — ' + Utilities.formatDate(ahora, TIMEZONE, 'HH:mm');

  // Parsear el HTML de Quill una sola vez → se usa en Doc y Calendar
  const lineas = parseHtmlALineas(contenido);

  // ── 1. Guardar en hoja Notas (HTML para la UI) ────────────────────────
  const sheetNotas = ss.getSheetByName('Notas');
  if (sheetNotas) {
    sheetNotas.insertRowBefore(2);
    sheetNotas.getRange(2, 1, 1, 4).setValues([[
      fechaStr, fechaHoraStr, etiqueta,
      contenido   // ← HTML: el panel "Hoy" lo renderiza como innerHTML
    ]]);
  }
  ...
  if (cfg.docId) {
    try {
      ...
      //   [HEADING2] 2026-04-18 12:58  ·  Seguimiento
      //   [NORMAL]   línea 1
      ...
      lineasInverso.forEach(linea => {
        ...
        if (linea.tipo === 'ol_item' || linea.tipo === 'ul_item') {
          // Sangría izquierda para ítems de lista
          p.setIndentStart(18);
      ...
      // Paso 3 — Título encabezado (queda primero de todo)
      const tituloNota    = fechaHoraStr + '  ·  ' + etiqueta;
      const parrafoTitulo = body.insertParagraph(0, tituloNota);
      parrafoTitulo.setHeading(DocumentApp.ParagraphHeading.HEADING2);

      doc.saveAndClose();
      docUrl = 'https://docs.google.com/document/d/' + cfg.docId;
    } catch (docErr) {
      Logger.log('Error al escribir en Doc v4: ' + docErr.message);
    }
  }

  // ── 3. Guardar en Google Calendar (texto plano estructurado) ──────────
  let calUrl = '';
  if (cfg.calendarId) {
    try {
      ...
      Logger.log('Error al escribir en Calendar v4: ' + calErr.message);
    }
  }
...
function setupHojas() {
  ...
  if (notas.getLastRow() === 0) {
    const notasHeaders = [['Fecha', 'FechaHora', 'Etiqueta', 'Contenido']];
    notas.getRange(1, 1, 1, 4).setValues(notasHeaders);
    notas.getRange(1, 1, 1, 4)
      .setBackground('#1e2128').setFontColor('#f0a500').setFontWeight('bold');
    notas.setFrozenRows(1);
    notas.setColumnWidth(1, 110);
    notas.setColumnWidth(2, 160);
    notas.setColumnWidth(3, 130);
    notas.setColumnWidth(4, 520);
  }
